...
from video.datasets.dataset_spheres_video import treat_data, TARGET_TO_PROMPT
...
def load_shared_models():
    global SHARED_VIDEO_DEPTH_MODEL, SHARED_INFERENCE_MODEL, SHARED_CONDITION_LIST, SHARED_FOCAL_LENGTH_MODEL
    ...
    if SHARED_INFERENCE_MODEL is None:
        SHARED_INFERENCE_MODEL = WanTrainingModule(
            model_paths=None,
        ...
        state_dict = load_state_dict("./checkpoints/Wan/step-250000.safetensors")
        load_result = SHARED_INFERENCE_MODEL.pipe.dit.load_state_dict(state_dict, strict=False)

        SHARED_INFERENCE_MODEL.to('cuda')
        SHARED_INFERENCE_MODEL.eval()
...
'''
    ...
    def predict_depth_and_focal(self, video_path: str, fov: float = 90.0, use_ml_focal_length: bool = True, num_frames: int = 21):
        ...
        try:
            ...
            if video_path.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff', '.webp')):
                # Load image and duplicate for specified number of frames
                ...
                # Read video frames (limit to specified number of frames)
    ...
    def run_inference_all(self, num_steps: int = 50, seed: int = 42):
        ...
        for sphere_key in sphere_types:
            sphere_prompt = TARGET_TO_PROMPT[sphere_key]
    ...
    def run_hdri_optimization(self, evs_str: str = "0,-3,-6,-9", num_iterations: int = 1000, 
                             ...
                             temporal_consistent: bool = False, temporal_loss_weight: float = 0.1, proj: str = "latlong"):
        ...
        try:
            ...
            optimization_log += f"  - Temporal consistent: {temporal_consistent}\n"
            if temporal_consistent:
                optimization_log += f"  - Temporal loss weight: {temporal_loss_weight}\n"
            optimization_log += "\n"
            ...
            if temporal_consistent:
                # Temporal optimization - all frames at once
                optimization_log += "Using temporal consistent optimization (all frames simultaneously)\n\n"
                
                # Load all frame data first
                frame_data = []
                
                for frame_idx in range(num_frames):
                    optimization_log += f"Loading frame {frame_idx + 1}/{num_frames}...\n"
                    
                    # Load camera info
                    world2cam = torch.eye(4).cuda()
                    
                    # Get geometry from processed data
                    if frame_idx >= len(self.processed_data["position_raw"]):
                        optimization_log += f"  Warning: Frame {frame_idx} out of range, skipping\n"
                        continue
                    
                    position_map = self.processed_data["position_raw"][frame_idx].cuda()
                    normal_map = self.processed_data["normals"][frame_idx].cuda()
                    mask = self.processed_data["sphere_mask"][frame_idx].cuda()
                    
                    normal_map[2,:,:] = -normal_map[2,:,:]
                    mask = (mask[0, :, :] > 0).float().unsqueeze(0)
                ...
                if len(frame_data) == 0:
                    return None, None, "No valid frames to process"
                
                optimization_log += f"Loaded {len(frame_data)} frames successfully\n\n"
                ...
                optimization_log += f"Initialized {len(pols)} environment maps with shape [3, 256, 512]\n"
                
                # Single optimizer for all frames
                optimizer = torch.optim.AdamW(all_params, betas=(0.9, 0.99), fused=True)
                
                # Optimization loop
                total_iterations = num_iterations * len(frame_data)
                optimization_log += f"Starting optimization for {total_iterations} iterations...\n\n"
                ...
                for i in range(total_iterations):
                    ...
                    recon_loss = fdata['weights'][random_sphere] * torch.mean(
                        torch.square((img - ref_img) * fdata['mask'] * mask_sat)
                    )
                    ...
                    if i % 100 == 0:
                        optimization_log += f"Iteration {i}/{total_iterations}, Recon Loss: {recon_loss.item():.6f}, "
                        optimization_log += f"Temporal Loss: {temporal_loss if isinstance(temporal_loss, float) else temporal_loss.item():.6f}, "
                        optimization_log += f"Total Loss: {loss.item():.6f}\n"
                ...
                for idx, fdata in enumerate(frame_data):
                    with torch.no_grad():
                        ...
                        hdr_path = os.path.join(hdr_output_dir, f"envmap_{fdata['frame']:04d}.exr")
                        imsave(hdr_path, envmap_hdr, compression='ZIP')
                        
                        # Save LDR preview
                        envmap_ldr = Lin_to_sRGB(recon).clamp(0, 1)
                        ldr_path = os.path.join(hdr_output_dir, f"envmap_ldr_{fdata['frame']:04d}.png")
                        torchvision.utils.save_image(envmap_ldr, ldr_path)
                        preview_images.append(ldr_path)
                        
                        optimization_log += f"Saved frame {fdata['frame']}: {hdr_path}\n"
                
            else:
                # Frame-by-frame optimization (original method)
                optimization_log += "Using frame-by-frame optimization\n\n"
                ...
                for frame_idx in range(num_frames):
                    ...
                    for i in range(num_iterations):
                        ...
                        if i % 10 == 0 and frame_idx == 0:
                            #Render both the envmap ldr and both spheres for frame 0
                            with torch.no_grad():
                                # Render both spheres
                                rendered_imgs = {}
                                for sphere in list_spheres:
                                    mode = modes[sphere]
                                    img_rendered = render(position_map, world2cam, 
                                                        normal_map, recon, 
                                                        mode=mode, num_samples=64, proj=proj)
                                    img_rendered = img_rendered #* (2**int(0))
                                    img_rendered = Lin_to_sRGB(img_rendered).clamp(0, 1)
                                    rendered_imgs[sphere] = img_rendered
                                
                                # Save preview images
                                vis_path_envmap = os.path.join(hdr_output_dir, f"preview_envmap")
                                os.makedirs(vis_path_envmap, exist_ok=True)
                                envmap_ldr = Lin_to_sRGB(recon).clamp(0, 1)
                                ldr_path = os.path.join(vis_path_envmap, f"preview_envmap_ldr_iter_{i:04d}.png")
                                torchvision.utils.save_image(envmap_ldr, ldr_path)
                                
                                for sphere in rendered_imgs.keys():
                                    vis_path_sphere = os.path.join(hdr_output_dir, f"preview_{sphere}")
                                    os.makedirs(vis_path_sphere, exist_ok=True)
                                    sphere_path = os.path.join(vis_path_sphere, f"preview_iter_{i:04d}.png")
                                    torchvision.utils.save_image(rendered_imgs[sphere], sphere_path)
                                
                                optimization_log += f"Saved preview images at iteration {i}\n"
    ...
    with gr.Tab("1. Depth Prediction"):
        with gr.Row():
            with gr.Column():
                ...
                with gr.Row():
                    num_frames_slider = gr.Slider(1, 81, value=21, step=1, label="Number of Frames")
                gr.Markdown("**Input**: Upload a video or image. Images will be duplicated to the specified number of frames.")
                ...
                gr.Markdown("**Frames**: For images, duplicate to this many frames. For videos, limit to this many frames.")
        ...
        predict_btn.click(
            ...
            inputs=[video_input, fov_slider, use_ml_focal, num_frames_slider],
  ...
  ...
    ...
    with gr.Tab("5. Inference"):
        with gr.Row():
            with gr.Column():
                sphere = gr.Dropdown(
                    choices=[TARGET_TO_PROMPT[key] for key in TARGET_TO_PROMPT.keys()],
                    value=TARGET_TO_PROMPT["sphere_0"],
                    label="Sphere"
                )
    ...
    with gr.Tab("6. Inference All"):
        ...
        with gr.Row():
            with gr.Column():
                num_steps_all = gr.Slider(10, 100, value=50, label="Inference Steps")
        ...
        **Requirements:**
        ...
        with gr.Row():
            with gr.Column():
                ...
                temporal_consistent = gr.Checkbox(
                    value=False,
                    label="Temporal Consistent Optimization",
                    info="Optimize all frames simultaneously with temporal consistency"
                )
                temporal_loss_weight = gr.Slider(
                    0.0, 1.0, value=0.1, step=0.01,
                    label="Temporal Loss Weight",
                    info="Weight for temporal consistency loss (only used when temporal consistent is enabled)",
                    visible=False
                )
        ...
        # Show/hide temporal loss weight based on temporal consistent checkbox
        temporal_consistent.change(
            fn=lambda x: gr.update(visible=x),
            inputs=[temporal_consistent],
            outputs=[temporal_loss_weight]
        )
        
        optimize_btn.click(
            demo_instance.run_hdri_optimization,
            inputs=[evs_input, num_iterations, learning_rate, temporal_consistent, temporal_loss_weight],
