...
class WanTrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        self.pipe.scheduler_train.set_timesteps(1000, training=True, shift=shift)
    ...
    def validation(self, data, inputs=None):
        ...
        video = self.pipe(data["prompt"], num_inference_steps=self.validation_steps, inputs_shared=inputs, sigma_shift=self.shift)
        return video
...
if __name__ == "__main__":
    ...
    dataset = SpheresDataset(config, target_layers=args.targets.split(","), num_frames=21)
    raise('ok')
    ...
    # Checkpoint resume logic - find checkpoint first
    first_step = 0
    ...
    if len(all_saved_ckpt) > 0:
        ...
        checkpoint_to_load = os.path.join(args.output_path, last_ckpt)
        if "step" in last_ckpt:
            first_step = int(last_ckpt.split("-")[1].split(".")[0])
        print(f"Found checkpoint {last_ckpt}, will resume from step {first_step}")
    else:
    ...
    # Load checkpoint AFTER model architecture is fully set up
    if checkpoint_to_load is not None:
        print(f"Loading checkpoint from {checkpoint_to_load}")
        state_dict = load_state_dict(checkpoint_to_load)
        
        # Load directly into the dit model (same as test.py)
        ...
        print(f"Successfully loaded checkpoint, resuming from step {first_step}")
    ...
    scheduler = get_scheduler(
        "cosine",
        optimizer=optimizer,
        num_warmup_steps=500 * 8,
        num_training_steps=args.num_steps * 8,
        num_cycles=1,
        power=1,
    )
