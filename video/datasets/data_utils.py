...
def depth_to_scaled_depth_vectorized(depth, alpha):
    """
    Vectorized version of depth_to_scaled_depth that handles batched inputs.
    depth: (B, 3, H, W) or (3, H, W)
    alpha: (B, 3, H, W) or (3, H, W)
    """
    if depth.dim() == 3:
        # Single tensor case - use original function
        return depth_to_scaled_depth(depth, alpha)
    ...
    for i in range(B):
        # Apply original function to each batch item
        log_masked_depth = torch.log10(depth[i, 0:3, alpha[i, 0] > 0.99])
        if log_masked_depth.numel() > 0:
            mean_log_depth = log_masked_depth.mean()
        else:
            mean_log_depth = torch.tensor(1.0, device=depth.device)
        
        # Shift the depth to have a log-mean of 1
        depth_shifted = depth[i, 0:3] / (10**(mean_log_depth - 1))
        filled_depth = depth_shifted + (1 - alpha[i]) * (10**3)
        log_depth = (torch.log10(filled_depth).clamp(-1, 3) + 1) / 4
        results.append(log_depth)
    
    return torch.stack(results, dim=0)

def position_to_scaled_position(position):
    """
    Convert position to scaled position.
    Supports both single tensors (C, H, W) and batched tensors (B, C, H, W).
    """
    if position.dim() == 3:
        # Single tensor case - use the commented out original logic
        linear_depth = position[2:, ...]
        linear_depth = linear_depth.repeat(3, 1, 1)
        log_depth = depth_to_scaled_depth(linear_depth, torch.ones_like(linear_depth))
        scaled_position = log_depth
        ...
        linear_depth = position[:, 2:, ...]  # Shape: (B, 1, H, W)
        linear_depth = linear_depth.repeat(1, 3, 1, 1)  # Shape: (B, 3, H, W)
        
        # Process each batch item individually since depth_to_scaled_depth doesn't handle batches
        results = []
        for i in range(B):
            log_depth = depth_to_scaled_depth(linear_depth[i], torch.ones_like(linear_depth[i]))
            results.append(log_depth)
        
        scaled_position = torch.stack(results, dim=0)  # Shape: (B, 3, H, W)
        scaled_position = torch.nan_to_num(scaled_position)
        return scaled_position
    else:
        raise ValueError(f"Unsupported tensor dimension: {position.dim()}. Expected 3 or 4 dimensions.")
