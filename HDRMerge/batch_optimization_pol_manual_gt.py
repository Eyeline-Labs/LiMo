...
def multi_importance_sample_bad(normals, envmap, num_samples=64):
    ...
    envmap_samples = envmap_samples * cosines[None] / (pdfs_selected[None] + 1e-8)
...
def render(position_map, world2cam, normal, envmap, mode='mirror', num_samples=16, proj='latlong'):
    ...
    if mode == 'mirror':
        ...
        #img = samples.mean(dim=2) * math.pi
    
    img = img.reshape(3, position_map.shape[1], position_map.shape[2])
    return img

def load_reference_images(sphere_out_path, evs, frame, sphere):
    """Load reference images for different exposure values."""
    ref_imgs = {}
    
    for ev in evs:
        # Try to load predicted image from test.py output
        pred_path = os.path.join(sphere_out_path, f'frame_{frame:04d}', sphere + f'_{ev}.png')
...
def load_reference_images_gt(sphere_path, evs, frame, sphere):
    """Load reference images for different exposure values."""
    ref_imgs = {}

    ref_img = imread(os.path.join(sphere_path, f"frame_{frame:04d}", sphere, f"image_{frame:04d}.exr")).astype(np.float32)[:,:,:3]
    ref_img = cv2.resize(ref_img, (512, 512), interpolation=cv2.INTER_LINEAR)
    ref_img = torch.from_numpy(ref_img).permute(2, 0, 1)  # Convert to CxHxW format
    
    for ev in evs:
        ref_img_ev = ref_img.clone() * (2 ** int(ev))
        ref_img_ev = Lin_to_sRGB(ref_img_ev).clamp(0,1)
        ref_imgs[ev] = ref_img_ev

    return ref_imgs

def optimize_envmap_for_scene(scene_path, data_path, out_path, evs, proj='latlong', mask_id=255):
    """Optimize environment map for a single scene."""
    print(f"Processing scene: {scene_path}")
    
    # Load camera info
    camera_info_path = os.path.join(scene_path, "frame_0001", "camera_info.pkl")
    ...
    for frame in range(1, num_frames + 1):
        ...
        ref_img_0 = load_reference_images_gt(scene_path, evs, frame, "sphere_0") 
        ref_img_1 = load_reference_images_gt(scene_path, evs, frame, "sphere_1")
        ...
        lr = 0.005
        pol = PoL(init, learning_rate=lr, weight_decay=0.05, padding_mode="circular").cuda()
        ...
        #list_spheres = [list_spheres[0]] #TEMP
        ...
        for i in range(1000):
            ...
            mask_sat = torch.ones_like(ref_img) # Create a mask for saturation
            threshold = 0.90
            #if mode == "mirror":
            mask_sat = torch.where((ref_img > threshold) & (img > threshold), 0.0, 1.0)

            loss = weights[random_sphere] * torch.mean(torch.square((img - ref_img.to(recon.device)) * mask * mask_sat))
            ...
            #     mode = modes[sphere]
            #     ref_dict = references[sphere]
            #     weight = weights[sphere]
                
            #     for ev in ref_dict.keys():
            #         ref_img = ref_dict[ev]
            #         ref_img_linear = sRGB_to_Lin(ref_img)
                    
            #         # Apply exposure correction
            #         exposure_scale = 2 ** (-int(ev))
            #         ref_img_linear = ref_img_linear / exposure_scale
                    
            #         # Render with current environment map
            #         img = render(position_map, world2cam, normal_map, recon, mode=mode, num_samples=32, proj=proj)
            #         img_masked = img * mask[0:1]
            #         ref_masked = ref_img_linear * mask[0:1]
                    
            #         # Compute loss
            #         sphere_loss = F.mse_loss(img_masked, ref_masked) * weight
            #         loss += sphere_loss
        ...
        imsave(os.path.join(frame_output_dir, f"envmap_{frame:04d}.exr"), envmap_hdr, compression='ZIP')
        print(os.path.join(frame_output_dir, f"envmap_{frame:04d}.exr"))
        ...
        # torchvision.utils.save_image(img, "diffuse_pred.png")
        ...
        # torchvision.utils.save_image(references["sphere_1"][ev], "diffuse_gt.png")

        # torchvision.utils.save_image(references["sphere_0"][ev], "mirror_gt.png")


        # envmap_ldr = Lin_to_sRGB(recon).clamp(0,1)
        # torchvision.utils.save_image(envmap_ldr, os.path.join(frame_output_dir, f"envmap_ldr_{frame:04d}.png"))
        
        print(f"Frame {frame} done in {time.time() - t_start:.2f} seconds")
        
        # # Use current result as initialization for next frame
        # init = recon.unsqueeze(0).detach()
        # init = torch.nan_to_num(init, nan=0.5)
        init = None
...
def main():
    ...
    scene_pattern = os.path.join(args.data_path, "lighting_0000", "*", "object_*")
    ...
    for scene_dir in tqdm(scene_dirs, desc="Processing scenes"):
        # if "camera_0000" not in scene_dir or "object_3" not in scene_dir:
        #     continue  # Skip irrelevant directories
